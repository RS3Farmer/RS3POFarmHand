export const RequiredImages: React.FC = () => {
  //Ardougne Images
  require("./../assets/Images/Allotment Herb Patches/ArdougneFarmingPatchReg.png");
  require("./../assets/Images/Allotment Herb Patches/ArdyAllotmentTopPatch.png");
  require("./../assets/Images/Allotment Herb Patches/ArdyAllotmentBottomPatch.png");
  require("./../assets/Images/Allotment Herb Patches/Ardy3dPatch.png");
  require("./../assets/Images/Allotment Herb Patches/HerbOrFlowerPatch.png");
  //Canifis and Falador Images
  require("./../assets/Images/Allotment Herb Patches/NECanifisPatchBareBones.png");
  require("./../assets/Images/Allotment Herb Patches/CanafisAllotmentCompleted.png");
  require("./../assets/Images/Allotment Herb Patches/CanafisAllotmentCompletedR.png");
  require("./../assets/Images/Allotment Herb Patches/CanifisPatch3d.png");
  require("./../assets/Images/Allotment Herb Patches/Falador3dPatch.png");
  //Catherby Images
  require("./../assets/Images/Allotment Herb Patches/Catherby3dPatch.png");
  require("./../assets/Images/Allotment Herb Patches/CatherByPatchBareBones.png");
  require("./../assets/Images/Allotment Herb Patches/Catherby3dPatch.png");
  //Harmony Island Images
  require("./../assets/Images/Allotment Herb Patches/HarmonyIsland3dPatch.png");
  require("./../assets/Images/Allotment Herb Patches/HarmonyIslandAllotmentOnlyBareBones.png");
  require("./../assets/Images/Allotment Herb Patches/HarmonyIslandAllotmentOnlyBareBonesCompleted.png");
  //Priffdinas Images
  require("./../assets/Images/Allotment Herb Patches/PriffHerbBushPatch.png");
  require("./../assets/Images/Allotment Herb Patches/HerbOrFlowerPatchReg.png");
  // Troll Stronghold Images
  require("./../assets/Images/Allotment Herb Patches/TrollheimHerbPatch3d.png");
  //Wilderness Images
  require("./../assets/Images/Allotment Herb Patches/Wildernesslvl22HerbPatch.png");
  require("./../assets/Images/Allotment Herb Patches/Wildernesslvl38HerbPatch3d.png");
  //Garnden of Kharid Images
  require("./../assets/Images/Allotment Herb Patches/GardenofKharidHerbPatch3d.png");
  //Ardougne South Bush Patch
  require("./../assets/Images/Bush Patches/ArdougneSouthBushPatch.png");
  //Champions Guild West Bush Patch
  require("./../assets/Images/Bush Patches/ChampionGuildBush3d.png");
  //Etceteria Bush Patch
  require("./../assets/Images/Bush Patches/escentiaBush3d.png");
  //Rimmington Bush Patch
  require("./../assets/Images/Bush Patches/RimmingtonBushPatch3d.png");
  //Alkharid Cactus Patch
  require("./../assets/Images/Cactus Patches/CactusPatchAlkharid3d.png");
  //Anachronia Cactus Patch
  require("./../assets/Images/Cactus Patches/CactusPatchanachronia3d.png");
  //Hets Oasis Cactus Patch
  require("./../assets/Images/Cactus Patches/HetsOasisCactusPatch3d.png");
  //Menaphos Cactus Patch
  require("./../assets/Images/Cactus Patches/MenaphosCactusPatch3d.png");
  //Brimhaven Fruit Tree Patch
  require("./../assets/Images/Fruit Tree Patches/BrimHavenFruitTreePatch3d.png");
  //Catherby Fruit Tree Patch
  require("./../assets/Images/Fruit Tree Patches/CatherbyFruitTreePatch3d.png");
  //Gnome Stronghold Fruit Tree Patch
  require("./../assets/Images/Fruit Tree Patches/FruitTreePatchGnomeStronghold3d.png");
  //Fruit Tree Patch Herblore Habitat
  require("./../assets/Images/Fruit Tree Patches/FruitTreePatchHerbloreHabitat.png");
  //Lletya Fruit Tree Patch
  require("./../assets/Images/Fruit Tree Patches/LletyaFruitTreePatch3d.png");
  //Gnome Village Fruit Tree Patch
  require("./../assets/Images/Fruit Tree Patches/FruitTreePatchWestGnomeVill3d.png");
  //Priffdinas Fruit Tree Patch
  require("./../assets/Images/Fruit Tree Patches/FruitTreePatchPriffdinas.png");
  //Champions Guild South-West Hops Patch
  require("./../assets/Images/Hops Patches/SEChampGuild3dHopsPatch.png");
  //Entrana Hops Patch
  require("./../assets/Images/Hops Patches/Entrana3dHopPatch.png");
  //Yanille Hops Patch
  require("./../assets/Images/Hops Patches/YanilleHopsPatch3d.png");
  //McGrubor Woods North Hops Patch
  require("./../assets/Images/Hops Patches/North McGruborsWoods.png");
  //Hops Patch Reg
  require("./../assets/Images/Hops Patches/HopsPatchReg.png");
  //Hops Patch Complete
  require("./../assets/Images/Hops Patches/HopsPatchComplete.png");
  //Canafis Mushroom Patch
  require("./../assets/Images/Mushroom Patches/CanifisMushroomPatch.png");
  //City of Um Mushroom Patch
  require("./../assets/Images/Mushroom Patches/cityofummushroompatch3d.png");
  //Tirannwn Mushroom Patch
  require("./../assets/Images/Mushroom Patches/TyrawynnMushroomPatch3d.png");
  //Mushroom Patch Bare Bones Reg
  require("./../assets/Images/Mushroom Patches/MushroomPatchBareBonesReg.png");
  //Mushroom Patch Bare Bones Complete
  require("./../assets/Images/Mushroom Patches/MushroomPatchBareBonesComplete.png");
  //Ardougne Images
  require("./../assets/Images/Special Patches/ArdougneSpecialPatch.png");
  require("./../assets/Images/Special Patches/ManorFarmSpecialPatches.png");
  //Anachronia Images
  require("./../assets/Images/Special Patches/VilebloomPatchAnachronia.png");
  //Draynor Images
  require("./../assets/Images/Special Patches/EvilturnipPatch3d.png");
  require("./../assets/Images/Special Patches/NightshadePatch3d.png");
  //Herblore Habitat Images
  require("./../assets/Images/Special Patches/HerbloreHabitatVineHerblorePatch.png");
  require("./../assets/Images/Special Patches/HerbloreHabitatVineHerblorePatch2.png");
  require("./../assets/Images/Special Patches/HerbloreHabitatFlowerPatch.png");
  require("./../assets/Images/Special Patches/HerbloreHabitatBushPatch.png");
  //Tai Bwo Wannai Images
  require("./../assets/Images/Special Patches/calquatpatch3d.png");
  //Priffdinas Images
  require("./../assets/Images/Special Patches/CrystalTreePatchPriff3d.png");
  require("./../assets/Images/Special Patches/PriffElderTreePatch3d.png");
  require("./../assets/Images/Special Patches/HarmoneyMoss.png");
  //Tuai Leit Images
  require("./../assets/Images/Special Patches/TuaiLeitArcBerryBushPatch3d.png");
  //Taverly Images
  require("./../assets/Images/Special Patches/PotatoePatchReg.png");
  require("./../assets/Images/Special Patches/PotatoePatchComplete.png");
  require("./../assets/Images/Special Patches/PotatoePatch.png");
  //Livid Farm Images
  require("./../assets/Images/Special Patches/SingleUsePlot3dLunarIsle.png");
  require("./../assets/Images/Special Patches/SingleUseAllotmentReg.png");
  require("./../assets/Images/Special Patches/SingleUseAllotmentComplete.png");
  //Port Sarim Spirit Tree Patch
  require("./../assets/Images/Spirit Tree Patches/SpiritTreePatchPortSarim3d.png");
  //Ardougne Spirit Tree Patch
  require("./../assets/Images/Spirit Tree Patches/ardougneTreePatch.png");
  //Escentia Spirit Tree Patch
  require("./../assets/Images/Spirit Tree Patches/EscentiaSpiritTreePatch.png");
  //Brimhaven Spirit Tree Patch
  require("./../assets/Images/Spirit Tree Patches/BrimhavenSpiritTreePatch3d.png");
  //Falador Tree Patch
  require("./../assets/Images/Tree Patches/TreePatchFalador3d.png");
  //Lumbridge Tree Patch
  require("./../assets/Images/Tree Patches/LumbridgeTreePatch3d.png");
  //Woodcutters Grove Tree Patch
  require("./../assets/Images/Tree Patches/WoodCuttersGroveTreePatch3d.png");
  //Taverly Tree Patch
  require("./../assets/Images/Tree Patches/TaverlyTreePatch3d.png");
  //Traehorn Tree Patch
  require("./../assets/Images/Tree Patches/TraehornTreePatch3d.png");
  //Gnome Stronghold Tree Patch
  require("./../assets/Images/Tree Patches/gnomeStrongholdTreePatch.png");
  //Tree Patch Reg
  require("./../assets/Images/Tree Patches/TreePatchReg.png");
  //Tree Patch Completed
  require("./../assets/Images/Tree Patches/TreePatchCompleted.png");
  //TeleportImages
  require("./../assets/Images/TeleportImages/AlkharidLodeStone.png");
  require("./../assets/Images/TeleportImages/AmuletOfGlory.png");
  require("./../assets/Images/TeleportImages/AnachroniaLodestone.png");
  require("./../assets/Images/TeleportImages/AnachroniaTeleport.png");
  require("./../assets/Images/TeleportImages/ArchaeologyBook.png");
  require("./../assets/Images/TeleportImages/ArcJournal.png");
  require("./../assets/Images/TeleportImages/ArdougneCloak1.png");
  require("./../assets/Images/TeleportImages/ArdougneLodestone.png");
  require("./../assets/Images/TeleportImages/ArdougneTeleport.png");
  require("./../assets/Images/TeleportImages/balloonSys.png");
  require("./../assets/Images/TeleportImages/BigBookOPiracy.png");
  require("./../assets/Images/TeleportImages/BroomStick.png");
  require("./../assets/Images/TeleportImages/CatherbyLodestone.png");
  require("./../assets/Images/TeleportImages/CamelotTeleport.png");
  require("./../assets/Images/TeleportImages/CanifisLodestone.png");
  require("./../assets/Images/TeleportImages/CanifisLodestone.png");
  require("./../assets/Images/TeleportImages/CapeOfLegends.png");
  require("./../assets/Images/TeleportImages/CaptainsLog.png");
  require("./../assets/Images/TeleportImages/CatherbyTeleport.png");
  require("./../assets/Images/TeleportImages/ChippedVarrockTeletab.png");
  require("./../assets/Images/TeleportImages/CharterShip.png");
  require("./../assets/Images/TeleportImages/CityofUmTele.png");
  require("./../assets/Images/TeleportImages/ClanVexillum.png");
  require("./../assets/Images/TeleportImages/ClockTowerTeleport.png");
  require("./../assets/Images/TeleportImages/CombatBracelet.png");
  require("./../assets/Images/TeleportImages/CrystalTeleportSeed.png");
  require("./../assets/Images/TeleportImages/DareeyakTeleport.png");
  require("./../assets/Images/TeleportImages/DraynorLodestone.png");
  require("./../assets/Images/TeleportImages/DungeoneeringCape(t).png");
  require("./../assets/Images/TeleportImages/Ectophial.png");
  require("./../assets/Images/TeleportImages/EdgevilleLever.png");
  require("./../assets/Images/TeleportImages/Enchanted_lyre.png");
  require("./../assets/Images/TeleportImages/ExplorersRing3.png");
  require("./../assets/Images/TeleportImages/ExplorersRing4.png");
  require("./../assets/Images/TeleportImages/FairyRingTeleAJR.png");
  require("./../assets/Images/TeleportImages/FairyRingTeleALQ.png");
  require("./../assets/Images/TeleportImages/FairyRingTeleALS.png");
  require("./../assets/Images/TeleportImages/FairyRingTeleBJP.png");
  require("./../assets/Images/TeleportImages/FairyRingTeleBLR.png");
  require("./../assets/Images/TeleportImages/FairyRingTeleCIQ.png");
  require("./../assets/Images/TeleportImages/FairyRingTeleCKR.png");
  require("./../assets/Images/TeleportImages/FairyRingTeleCKS.png");
  require("./../assets/Images/TeleportImages/FairyRingTeleDJP.png");
  require("./../assets/Images/TeleportImages/FaladorLodestone.png");
  require("./../assets/Images/TeleportImages/FaladorTeleport.png");
  require("./../assets/Images/TeleportImages/FarmersHat.png");
  require("./../assets/Images/TeleportImages/FirealtarTeleportdetail.png");
  require("./../assets/Images/TeleportImages/FishingGuildTeleport.png");
  require("./../assets/Images/TeleportImages/FortForinthryLodestone.png");
  require("./../assets/Images/TeleportImages/FremnikLodestone.png");
  require("./../assets/Images/TeleportImages/GnomeGlider.png");
  require("./../assets/Images/TeleportImages/GrandSeedPod.png");
  require("./../assets/Images/TeleportImages/HazlemeresSignetRing.png");
  require("./../assets/Images/TeleportImages/HomeTeleport.png");
  require("./../assets/Images/TeleportImages/HouseTeleport.png");
  require("./../assets/Images/TeleportImages/ImperialDistrictTeleport.png");
  require("./../assets/Images/TeleportImages/JujuTeleportBag.png");
  require("./../assets/Images/TeleportImages/KaramjaGloves3.png");
  require("./../assets/Images/TeleportImages/KharyllTeleport.png");
  require("./../assets/Images/TeleportImages/KaramjaLodeStone.png");
  require("./../assets/Images/TeleportImages/LumbridgeLodestone.png");
  require("./../assets/Images/TeleportImages/LumbridgeTeleport.png");
  require("./../assets/Images/TeleportImages/LunarIsleLodestone.png");
  require("./../assets/Images/TeleportImages/ManorFarmTeleport.png");
  require("./../assets/Images/TeleportImages/MasterArchaeologistsOutfit.png");
  require("./../assets/Images/TeleportImages/MasterCamoOutFit.png");
  require("./../assets/Images/TeleportImages/MenaphosLodestone.png");
  require("./../assets/Images/TeleportImages/MiscellaniaTeleport.png");
  require("./../assets/Images/TeleportImages/MisterGully.png");
  require("./../assets/Images/TeleportImages/ModifiedBlacksmithsHelmet.png");
  require("./../assets/Images/TeleportImages/ModifiedBotonistsMask.png");
  require("./../assets/Images/TeleportImages/MoonClanTeleport.png");
  require("./../assets/Images/TeleportImages/MysticalSandSeed.png");
  require("./../assets/Images/TeleportImages/NatureSentinelOutfit.png");
  require("./../assets/Images/TeleportImages/NorthArdougneTele.png");
  require("./../assets/Images/TeleportImages/ObeliskShard.png");
  require("./../assets/Images/TeleportImages/OrthernTeleportation.png");
  require("./../assets/Images/TeleportImages/PirateSpell.png");
  require("./../assets/Images/TeleportImages/PortSarimlodestone.png");
  require("./../assets/Images/TeleportImages/PortableObelisk.png");
  require("./../assets/Images/TeleportImages/PriffdinasCamoOutFit.png");
  require("./../assets/Images/TeleportImages/PriffdinasLodestone.png");
  require("./../assets/Images/TeleportImages/RemorasNecklace.png");
  require("./../assets/Images/TeleportImages/RingOfDueling.png");
  require("./../assets/Images/TeleportImages/RingOfWealth.png");
  require("./../assets/Images/TeleportImages/Rum.png");
  require("./../assets/Images/TeleportImages/SandSeed.png");
  require("./../assets/Images/TeleportImages/SceptreOfTheGods.png");
  require("./../assets/Images/TeleportImages/SealOfPassage.png");
  require("./../assets/Images/TeleportImages/SeersVillageLodestone.png");
  require("./../assets/Images/TeleportImages/SkillsNecklace.png");
  require("./../assets/Images/TeleportImages/SlayerCape(t).png");
  require("./../assets/Images/TeleportImages/SouthFaladorTeleport.png");
  require("./../assets/Images/TeleportImages/SpiritTree.png");
  require("./../assets/Images/TeleportImages/TaiBwoWannaiTeleport.png");
  require("./../assets/Images/TeleportImages/TotemOfTheAbyss.png");
  require("./../assets/Images/TeleportImages/TrollheimFarmTeleport.png");
  require("./../assets/Images/TeleportImages/TaverlyLodestone.png");
  require("./../assets/Images/TeleportImages/TaverlyTeleport.png");
  require("./../assets/Images/TeleportImages/TeleportToHouse.png");
  require("./../assets/Images/TeleportImages/TirannwnLodestone.png");
  require("./../assets/Images/TeleportImages/TirannwnQuiver3.png");
  require("./../assets/Images/TeleportImages/TirannwnQuiver4.png");
  require("./../assets/Images/TeleportImages/TomeOfUm1.png");
  require("./../assets/Images/TeleportImages/TrollheimAgilitySC.png");
  require("./../assets/Images/TeleportImages/TrollheimTeleport.png");
  require("./../assets/Images/TeleportImages/TuaiLeitTeleport.png");
  require("./../assets/Images/TeleportImages/VarrockLodestone.png");
  require("./../assets/Images/TeleportImages/VarrockTeleport.png");
  require("./../assets/Images/TeleportImages/Waka.png");
  require("./../assets/Images/TeleportImages/WickedHood.png");
  require("./../assets/Images/TeleportImages/WildernessLodestone.png");
  require("./../assets/Images/TeleportImages/WildernessSword1.png");
  require("./../assets/Images/TeleportImages/WildernessSword2.png");
  require("./../assets/Images/TeleportImages/WitchDoctorsMask.png");
  require("./../assets/Images/TeleportImages/YanilleLodestone.png");
  //Required Images for POF
  require("./../assets/PatchData/PatchTeleports.json");
  require("./../assets/POFAnimalData/AnimalBreedingRates.json");
  require("./../assets/POFAnimalData/AnimalFeedingInfo.json");
  require("./../assets/POFAnimalData/AnimalGoalOutput.json");
  require("./../assets/POFAnimalData/AnimalGrowthTime.json");
  require("./../assets/POFAnimalData/AnimalPenType.json");
  require("./../assets/POFAnimalData/AnimalSeasonSpecific.json");
  require("./../assets/POFAnimalData/AnimalSellingTraits.json");
  require("./../assets/POFAnimalData/AnimalTraits.json");
  require("./../assets/POFAnimalData/AnimalTroughInfo.json");
  require("./../assets/POFAnimalData/Animals.json");
  require("./../assets/POFAnimalData/BreedingTraitSpecific.json");
  require("./../assets/POFAnimalData/BuyersInfo.json");
  require("./../assets/POFAnimalData/SellingBuyersPercentages.json");
  require("./../assets/POFAnimalData/SellingOutfitPercentage.json");
  require("./../assets/POFAnimalData/SellingPricesOfAnimals.json");
  require("./../assets/POFAnimalData/SellingRankPercentages.json");
  require("./../assets/SeedData/AllotmentSeeds.json");
  require("./../assets/SeedData/BushSeeds.json");
  require("./../assets/SeedData/CactusSeeds.json");
  require("./../assets/SeedData/CombatSeeds.json");
  require("./../assets/SeedData/FlowerSeeds.json");
  require("./../assets/SeedData/FruitTreeSeeds.json");
  require("./../assets/SeedData/HerbSeeds.json");
  require("./../assets/SeedData/HopSeeds.json");
  require("./../assets/SeedData/JujuBushSeeds.json");
  require("./../assets/SeedData/JujuFlowerSeeds.json");
  require("./../assets/SeedData/MushroomSeeds.json");
  require("./../assets/SeedData/SpecialSeeds.json");
  require("./../assets/SeedData/TreeSeeds.json");

  return null;
};
